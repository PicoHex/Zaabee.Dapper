using System;
using Xunit;
using Zaabee.Dapper.Lambda.TestProject.Models;
...
namespace Zaabee.Dapper.Lambda.TestProject
{
	public class UnitTest1
	{
		...
		public void Test1()
		{
			...
			Printf(
				Expre2Sql.Select<UserInfo>(u => u.Id).Where(u => u.Name.Like("b")),
				"查询单表，带where Like条件"
			);


			Printf(
				Expre2Sql.Select<UserInfo>(u => u.Id).Where(u => u.Name.LikeLeft("b")),
				"查询单表，带where LikeLeft条件"
			);


			Printf(
				Expre2Sql.Select<UserInfo>(u => u.Id).Where(u => u.Name.LikeRight("b")),
				"查询单表，带where LikeRight条件"
			);


			Printf(
				Expre2Sql.Select<UserInfo>(u => u.Name).Where(u => u.Id.In(1, 2, 3)),
				"查询单表，带where in条件，写法一"
			);

			int[] aryId = {1, 2, 3};
			Printf(
				Expre2Sql.Select<UserInfo>(u => u.Name).Where(u => u.Id.In(aryId)),
			...
			Printf(
				Expre2Sql.Select<UserInfo>(u => u.Id).Where(
					u => u.Name == "b"
					     && u.Id > 2
					     && u.Name != null
					     && u.Id > int.MinValue
			...
			Printf(
				Expre2Sql.Select<UserInfo, Account, Student, Class, City, Country>((u, a, s, d, e, f) =>
					...
					.Join<Account>((u, a) => u.Id == a.UserId).LeftJoin<Account, Student>((a, s) => a.Id == s.AccountId)
					.RightJoin<Student, Class>((s, c) => s.Id == c.UserId)
					.InnerJoin<Class, City>((c, d) => c.CityId == d.Id)
					.FullJoin<City, Country>((c, d) => c.CountryId == d.Id).Where(u => u.Id != null),
				"多表复杂关联查询"
			);


			Printf(
				Expre2Sql.Select<UserInfo>().GroupBy(u => u.Name),
				"GroupBy分组查询"
			);


			Printf(
				Expre2Sql.Select<UserInfo>().OrderBy(u => u.Id),
				"OrderBy排序"
			);
			...
			Printf(
				Expre2Sql.Delete<UserInfo>().Where(u => u.Id == null),
				"根据where条件删除指定表记录"
			);
		...
		private static void Printf<T>(Expression2SqlCore<T> expression2Sql, string description = "")
		{
			...
			foreach (var item in expression2Sql.DbParams)
